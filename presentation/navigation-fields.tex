...
\begin{frame}
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item $d_{\osmDestination}(\uu)$, gesuchte Länge des kürzesten Wegs von $\uu$ nach $\osmDestination$
		\item $d(\uu, \vvv)$, Länge/Gewicht der Kante $(\uu, \vvv)$
...
\begin{frame}[fragile]
	\frametitle{Navigation auf dem Gitter: Dijkstra's Algorithmus}
		...
		\begin{column}{0.6\textwidth}
			...
			\end{enumerate}}
			\vspace{1cm}
			\uncover<2->{\textbf{Komplexität:} (einfacher Graph mit positiven Kosten und $n$ Knoten)
			\begin{enumerate}[label=$\bullet$]
...
\begin{frame}
	...
	\textbf{Beobachtung}: Ist $\uu_0, \ldots, \uu_k, \ldots,\uu_m$ der kürzeste Weg von $\uu_0$ nach $\uu_m$, dann ist
	\begin{equation*}
		\uu_0, \ldots, \uu_k
	\end{equation*}
	der kürzeste Weg von $\uu_0$ nach $\uu_k$.\\
	\vspace{1cm}
	
	\uncover<2->{\textbf{Invarianz}: Für alle $\uu \in \domain$, die aus $\mathcal{Q}$ gelöscht werden, ist $d_{\osmDestination}(\uu)$ die Länge des kürzesten Wegs von $\uu$ nach $\osmDestination$. (Beweis über Induktion)}
\end{frame}
...
\begin{frame}
	\frametitle{Navigation auf dem Gitter: A$^*$ Algorithmus}
	\textbf{Strategie}: Berechne gerichtet/informiert die kürzesten Wege von $\Gamma$ zu Gitterpunkten $\uu \in \domain$. Spare dir unnötige Berechnungen (\textsc{A$^*$} \cite{hart-1968})\\
	\vspace{1cm}
...
\begin{enumerate}[label=(\roman*)]
	...
	\uncover<3->{\item Für die Viskositätslösung muss $T$ nicht überall differenzierbar sein}
	\uncover<3->{\item Gilt $f = 1$, so ist $T$ die \textbf{geodätische Distanz}.}
\end{enumerate}
\end{frame}
...
\begin{frame}
	...
	\end{columns}
	\vspace{0.5cm}
	\uncover<3->{Wüssten wir, dass die Welle von unten rechts kommt bräuchten wir nur 
	\begin{equation*}
		D^{+ x}_{i,j}\uu = \frac{
